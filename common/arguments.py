...
def parse_args():
    ...
    # General arguments
    parser.add_argument(
        "-d", "--dataset", default="h36m", type=str, metavar="NAME", help="target dataset"
    )  # h36m or humaneva
    parser.add_argument(
    ...
    parser.add_argument(
        "-ste", "--subjects-test", default="S9,S11", type=str, metavar="LIST", help="test subjects separated by comma"
    )
    parser.add_argument(
        "-sun",
        "--subjects-unlabeled",
    ...
    parser.add_argument(
        "-c", "--checkpoint", default="checkpoint", type=str, metavar="PATH", help="checkpoint directory"
    )
    parser.add_argument(
        "--checkpoint-frequency", default=40, type=int, metavar="N", help="create a checkpoint every N epochs"
    )
    parser.add_argument(
        "-r", "--resume", default="", type=str, metavar="FILENAME", help="checkpoint to resume (file name)"
    )
    parser.add_argument(
        "--evaluate", default="", type=str, metavar="FILENAME", help="checkpoint to evaluate (file name)"
    )
    ...
    parser.add_argument(
        "-b", "--batch-size", default=1024, type=int, metavar="N", help="batch size in terms of predicted frames"
    )
    parser.add_argument("-drop", "--dropout", default=0.0, type=float, metavar="P", help="dropout probability")
    parser.add_argument(
        "-lr", "--learning-rate", default=0.0001, type=float, metavar="LR", help="initial learning rate"
    )
    parser.add_argument(
        "-lrd", "--lr-decay", default=0.99, type=float, metavar="LR", help="learning rate decay per epoch"
    )
    parser.add_argument(
        "-no-da",
    ...
    parser.add_argument(
        "-frame", "--number-of-frames", default="81", type=int, metavar="N", help="how many frames used as input"
    )
    parser.add_argument(
        "-frame-kept", "--number-of-kept-frames", default="27", type=int, metavar="N", help="how many frames are kept"
    )
    parser.add_argument(
        "-coeff-kept", "--number-of-kept-coeffs", type=int, metavar="N", help="how many coefficients are kept"
    )
    ...
    parser.add_argument(
        "--downsample", default=1, type=int, metavar="FACTOR", help="downsample frame rate by factor (semi-supervised)"
    )
    parser.add_argument("--warmup", default=1, type=int, metavar="N", help="warm-up epochs for semi-supervision")
    parser.add_argument(
        "--no-eval", action="store_true", help="disable epoch evaluation while training (small speed-up)"
    )
    parser.add_argument("--dense", action="store_true", help="use dense convolutions instead of dilated convolutions")
    parser.add_argument(
        "--disable-optimizations", action="store_true", help="disable optimized model for single-frame predictions"
    )
    parser.add_argument(
        "--linear-projection", action="store_true", help="use only linear coefficients for semi-supervised projection"
    )
    parser.add_argument(
        "--no-bone-length",
        action="store_false",
